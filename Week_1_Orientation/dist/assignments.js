...
function findHashWithPrefix(prefix) {
    ...
    while (true) {
        ...
        const hashStr = crypto_1.default.createHash('sha256').update(inp).digest('hex');
...
function findInputSting(inpStr, prefix) {
    let i = 0;
    while (true) {
...
// Ram => Ankit | Rs 10
function findNounce(input, prefix) {
    let i = 0;
    while (true) {
        const inputStr = input + i.toString();
        const hash = crypto_1.default.createHash('sha256').update(inputStr).digest('hex');
